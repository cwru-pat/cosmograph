...
// then assigning mapping function Dx(s) so that each phase space voxel
// has the same mass.
// In order to implement this, it follows the following steps:
// 1. calculate the total mass by iterating \rho(x) through x space;
// 2. calculate mass in each voxel by assuming each of them carries equal mass;
// 3. integrating \rho(x) through x again, assigning mapping function whenever
