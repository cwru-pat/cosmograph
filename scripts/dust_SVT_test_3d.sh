...
do_runs () {
  ...
  sed -i -E "s/ic_type = [[:alnum:]_\./]+/ic_type = sinusoid_3d/g" $TMP_CONFIG_FILE
  
  DIR="dust_run-3D-L_$BOX_LENGTH-r_$RES_STR-Odx_$METHOD_ORDER-A_$MODE_AMPLITUDE-GAUGE_$GAUGE"
  sed -i -E "s,output_dir = [[:alnum:]_-\./]+,output_dir = $DIR,g" $TMP_CONFIG_FILE
  mkdir -p $DIR
  printf "Performing run with settings in dir $DIR.\n"

  cmake .. -DCOSMO_N=$RES_INT -DCOSMO_STENCIL_ORDER=$METHOD_ORDER -DCOSMO_USE_REFERENCE_FRW=0 -DCOSMO_H_LEN_FRAC=$BOX_LENGTH -DCOSMO_USE_Z4c_DAMPING=0 && make -j
  if [ $? -ne 0 ]; then
...
# r = 0.005
do_runs $L "0032" 32 1 1 4 $A $Gauge $DT_FRAC
do_runs $L "0048" 32 1 1 4 $A $Gauge $DT_FRAC
do_runs $L "0064" 32 1 1 4 $A $Gauge $DT_FRAC
