...
do
  case $i in
      ...
      printf "         [-C|--cluster-run] [(-N|--resolution-N)=128] [-d|--dry-run] [(-f|--flip-delay)=0]\n"
      exit 0
      ;;
      -N=*|--resolution-N=*)
      RES="${i#*=}"
...
# Job directory
JOBDIR="dust_shell_R-${RES}_F-${FLIP_STEP}"
...
sed -i.bak "s/steps = 1600/steps = $STEPS/" config.txt
sed -i.bak "s/ray_flip_step = 800/ray_flip_step = $FLIP_STEP/" config.txt
sed -i.bak "s/IO_3D_grid_interval = 100/IO_3D_grid_interval = $IO3D/" config.txt
